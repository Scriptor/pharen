...
$__scope_id = Lexical::init_closure("repl", 191);
...
Lexical::bind_lexing("repl", 191, '$greetings', $greetings);
...
function repl__lambdafunc21($func, $__closure_id){
...
function strip_ns($funcs){
	...
	return map(new \PharenLambda("pharen_repl\\repl__lambdafunc21", Lexical::get_closure_id("repl", Null)), $funcs);
}

function repl__partial0($arg0, $__closure_id){
	$needle = Lexical::get_lexical_binding('repl', 200, '$needle', isset($__closure_id)?$__closure_id:0);;
	return starts_with($needle, $arg0);
}

function pharen_complete_func($input){
		$__scope_id = Lexical::init_closure("repl", 200);
	$all_funcs = get_defined_functions();
	$needle = get_needle($input);
		Lexical::bind_lexing("repl", 200, '$needle', $needle);
...
function get_prompt($expecting){
	...
	function repl__lambdafunc22($ns, $__closure_id){
		if((count($ns) == 2)){
...
function add_uses($code){
	...
	if(get(\RootNode::$ns, \RootNode::$uses)){
		...
		$__condtmpvar5 = reduce_to_str(new \PharenLambda("pharen_repl\\repl__lambdafunc22", Lexical::get_closure_id("repl", Null)), $uses);
	}
	else{
		$__condtmpvar5 = "";
	}
...
function intro(){
	$greetings = Lexical::get_lexical_binding('repl', 191, '$greetings', isset($__closure_id)?$__closure_id:0);;
